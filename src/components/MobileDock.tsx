import { site } from '../data/siteContent';
import { useQuoteLauncher } from './quote/quoteContext';

/** Barra fixa do celular, sempre ao alcance do polegar. */
export function MobileDock() {
  const { openQuote } = useQuoteLauncher();

  return (
    <div className="dock">
      <button type="button" className="btn btn--primary dock__cta" onClick={() => openQuote({ source: 'dock' })}>
        Montar meu orçamento
      </button>
      <a
        className="dock__whatsapp"
        href={site.whatsapp.href}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={`Falar no WhatsApp ${site.whatsapp.display}`}
      >
        <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true" focusable="false">
          <path
            fill="currentColor"
            d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.9 9.9 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.82 9.82 0 0 0 12.04 2Zm4.52 11.98c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.16.24-.64.8-.79.97-.14.16-.29.19-.54.06-.25-.12-1.05-.39-2-1.23-.74-.66-1.24-1.47-1.38-1.72-.15-.25-.02-.38.11-.5.11-.11.25-.29.37-.43.12-.15.16-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.4-.42-.56-.43h-.48c-.17 0-.43.06-.66.31-.23.25-.86.85-.86 2.07 0 1.21.89 2.39 1.01 2.55.12.17 1.75 2.67 4.23 3.74.59.26 1.05.41 1.41.52.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.68-1.18.21-.58.21-1.07.14-1.18-.06-.11-.22-.17-.47-.29Z"
          />
        </svg>
      </a>
    </div>
  );
}
